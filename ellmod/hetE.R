...
# Local function, plots output of hetE analyses...
...
#
		...
		gout = approx(gs$x,gs$y,xout=u)$y # output smooth monotonic regression 
		#
		par(mfrow=c(2,1))
		# uptake profiles
		ylimi=range(c(v[is,c(3,2)]))
		matplot(u,v[is,c(3,2)],
...
#
	...
	oo = order(iso$ord)
	keep = cbind(tx,approx(iso$x,iso$yf[oo],xout=tx,rule=2)$y,-y)
	return(keep)
}
...
#
	...
	if(doplot){
		matplot(sort(v[,1]),v[order(v[,1]),c(3,2)],type="pl",pch=".")
	}
	yh0 = v[,2] 
	nls.cv='FALSE' 
	aa=NULL 
...
#
	# Mode point -> mx
	out = smooth.spline(x,y,w=w,spar=.75) 
	sx = out$x 
	sy = out$y
	low = quantile(x,.025) 
	high = quantile(x,.975) 
	...
	for(j in 1:(ns-1)){ 
 	...
 	if(length(dfns)>1){
 		lam = approx(dfns,lamn,xout=min(n,nx)-.001,rule=2)$y
 	}
...
#
	ur = unismooth(u,y)
	bs = smooth.spline(ur,df=ddf)
	gb = approx(bs$x,bs$y,xout=u,rule=2)$y 
...
# 
	...
	grad = (approx(u,yh,xout=u*1.1,rule=2)$y-approx(u,yh,xout=u*.9,rule=2)$y)/du
